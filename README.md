...
- 使用MUI框架
小细节:
1.  scoped实现原理：通过css属性选择器，在上级div设置了一个选择器的属性,默认情况下在组件里使用scoped的话,会把样式隔离,也就是说在这里写样式只影响当前组件,如果你的组件里导入的有其他组件的话,在这里写样式是不生效的.
    + 如果想让其他组件生效只能再加个`<style><style>`里面写全局样式了...或者使用全局样式组件`sass-resources-loader`
...
## 开始
    ```html里注意配置下meta,要不然浏览器手机模式不配置```
### one轮播图+6宫格+首页切换组件
0. 6宫格注意样式使用`min-width:33.3% + flex布局`让图标上下平均分配.
1. 拷贝文件，执行`npm i`根据配置文件安装环境了，清理下路由和不需要的文件。
2. 按需导入，header和底部，一个是组件，一个是MUI，修改header的样式
3. 把项目托管到github--记得把那几个不需要提交的文件标识下，密钥、地址之类的。git pust 什么的
...
5. 懒加载图片样式里需要定义,剩下的就是写全部里的样式了，懒加载需要加载mint-ui所有组件不再指定导入了。记得导入css
6. 点击图片跳转进入详情，定义`router-link`在循环的li里,to理由里除了到每个图片的info参数要加图片ID，li下a改route-link的时候样式变了，使用tag指定标签。
7. 创建photoinfo,在路由里指定路径，记得加参数。写详情页，获取图片详情根据路由上参数ID`this.$route.params.id`，去调接口获取数据放到图片详情里
8. 把获取的详情页写样式，加上评论子组件（导入组件，注册组件，放到页面中），如果有接口，评论子组件上得把当前获取的图片的id传过去，评论子组件上写的有根据ID去请求接口数据获取评论,父子传递数据，这里没接口没法演示。
9. 图片缩略图需要用到一个插件`vue-previwe`图片预览插件，调用接口的缩略图，使用v-for进行渲染，img里的class属性不能动，每个图片对象数据里需要有w h属性，给缩略图套个DIV，写样式---自己写个例子找三方的接口来调用。实现缩略图的功能

### 使用真实手机测试
1. 在package.json的热加载配置里，在--host 后面写上你的IP地址，手机和项目电脑在同一局域网。直接访问IP端口即可。
### five商品列表
1. 写商品的路由跳转。创建goods文件夹下的goodlists的组件，实现经典的两列布局。着重了解flex...
2. 使用接口调用商品列表，加载到页面，制作加载更多按钮，点击加载更多 id++ 再调用接口，追加到商品列表里。
3. **编程式导航** 不是用vue-router的路由导航，使用JS的形式进行路由导航，也就是不使用`<route-link>`的方式，根据触发的事件进行跳转。
    + 根据vue-router的文档，使用路由导航，调用的时候使用`this.$trouter.push('/home/xxx/)`进行跳转。
    + 其中：this.$route和$this.router都在Vue对象中，打印this可以看到。this.$route是路由【参数对象】，所有路由中的参数，params,query都属于它
    + 其中：this.$router是一个路由【导航对象】，用它可以方便的时候JS代码，实现路由的前进、后退、跳转到新的URL地址，使用简单方式
4. 返回按钮，使用$router.go = -1实现     
